...
- (void)setupDockIcon
{
  ...
  // Calculate the x and y position to center the ActiveLight horizontally and place it at the
  // bottom
  ...
  NSRect activeLightFrame =
      NSMakeRect(xPosition, yPosition, self.activeLightDiameter, self.activeLightDiameter);
...
- (void)mouseDragged:(NSEvent *)event
{
  ...
  self.dragWindow = [[NSWindow alloc]
      initWithContentRect:NSMakeRect(dragLocation.x, dragLocation.y, newSize.width, newSize.height)
  ...
  NSImageView *imageView =
      [[NSImageView alloc] initWithFrame:NSMakeRect(0, 0, newSize.width, newSize.height)];
  ...
  DockGroup *parentView =
      self.superview; // Need this because compiler thinks this references NSView
  NSString *gName = [parentView getGroupName];

  // Broadcast that dragging is about to start
  [[NSNotificationCenter defaultCenter]
      postNotificationName:@"DockIconIsAboutToDragNotification"
                    object:self
                  userInfo:@{
  ...
  while ([event type] != NSEventTypeLeftMouseUp) {
    ...
    [[NSNotificationCenter defaultCenter]
        postNotificationName:@"DockIconIsDraggingNotification"
                      object:self
                    userInfo:@{
                      @"appName" : self.appName,
                      @"parentGroup" : [parentView getGroupName],
  ...
  if (NSPointInRect(newDragLocation, [[self window] frame])) {
    ...
    [[NSNotificationCenter defaultCenter]
        postNotificationName:@"DockIconAddedToGroupNotification"
                      object:self
                    userInfo:@{ @"appName" : self.appName, @"groupName" : gName }];
  }
  else if (!NSPointInRect(newDragLocation, [[self window] frame]) && parentView.canDragRemove) {
    // Dropped outside Dock app window
    [[NSNotificationCenter defaultCenter]
        postNotificationName:@"DockIconRemovedFromWindowNotification"
                      object:self
                    userInfo:@{
                      @"appName" : self.appName,
