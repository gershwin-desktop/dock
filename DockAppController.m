...
- (instancetype)init
{
  ...
  if (self) {
    ...
    _runningDivider = nil;
    _defaultIcons =
        [NSArray arrayWithObjects:/*@"Workspace",*/ @"Terminal", @"SystemPreferences", nil];
    ...
    _workspace =
        [NSWorkspace sharedWorkspace]; // Initialize the workspace property with the shared instance
    _iconSize = 64;
    _activeLight = 10;
    _padding = 16;
    ...
    // Subscribe to NSWorkspaceDidActivateApplicationNotification: Sent when an application is
    // terminated.
    [workspaceNotificationCenter addObserver:self
    ...
    // Subscribe to NSApplicationDidBecomeActiveNotification: Sent when an application becomes
    // active.
...
- (void)loadDockedIconsFromUserDefaults
{
  ...
  if (self.showDocked && [retrievedDockedIcons count] > 0) {
    for (NSUInteger i = 0; i < [retrievedDockedIcons count]; i++) {
      ...
      [self.dockedGroup addIcon:[retrievedDockedIcons objectAtIndex:i]
                      withImage:[self.workspace appIconForApp:iconName]
                        atIndex:i];
    }
    ...
    for (NSUInteger i = 0; i < [self.defaultIcons count]; i++) {
      ...
      [self.dockedGroup addIcon:[self.defaultIcons objectAtIndex:i]
                      withImage:[self.workspace appIconForApp:iconName]
                        atIndex:i];
    }
    // [self updateDockWindow];
...
- (void)setupDockWindow
{
  ...
  NSRect frame =
      NSMakeRect(x, 16, dockWidth, dockHeight); // Set size and position of the dock (x, y, w, h)
  ...
  if (self.fileManagerAppName) {
    ...
    [self.fileManagerGroup addIcon:self.fileManagerAppName
                         withImage:appIcon
                           atIndex:[[self.fileManagerGroup listIconNames] count]];
  }
  ...
  if (self.showDocked) {
    ...
    NSRect newDockedDividerFrame =
        NSMakeRect(0, self.padding, dockedDividerFrame.size.width, dockedDividerFrame.size.height);
  ...
  if (self.showRunning) {
    ...
    NSRect newDockedDividerFrame = NSMakeRect(0, self.padding, runningDividerFrame.size.width,
                                              runningDividerFrame.size.height);
  ...
  if (self) {
    ...
    // Fetch Docked Apps from Prefs
    [self.trashGroup addIcon:@"Trash"
                   withImage:trashIcon
                     atIndex:[[self.trashGroup listIconNames] count]];
  }
...
- (CGFloat)calculateDockWidth
{
  ...
  CGFloat dockWidth =
      (self.padding * 2) + (totalIcons * self.iconSize) + totalDividers * (self.padding * 2 + 1);
  return dockWidth;
}
...
- (void)updateDockWindow
{
  ...
  if (self.fileManagerGroup) {
    ...
    NSRect newDockedFrame = NSMakeRect(newGroupX, fileManagerFrame.origin.y,
                                       fileManagerFrame.size.width, fileManagerFrame.size.height);
  ...
  if (self.showDocked && self.dockedGroup) {
    ...
    NSRect newDockedFrame = NSMakeRect(newGroupX, dockedFrame.origin.y, dockedFrame.size.width,
                                       dockedFrame.size.height);
    ...
    NSRect dockedDividerFrame = [self.dockedDivider frame];
    NSRect newDockedDividerFrame =
        NSMakeRect(newGroupX, dockedDividerFrame.origin.y, dockedDividerFrame.size.width,
                   dockedDividerFrame.size.height);
    [self.dockedDivider setFrame:newDockedDividerFrame];
  ...
  if (self.showRunning && self.runningGroup) {
    ...
    NSRect newRunningFrame = NSMakeRect(newGroupX, runningFrame.origin.y, runningFrame.size.width,
                                        runningFrame.size.height);
    ...
    NSRect runningDividerFrame = [self.runningDivider frame];
    NSRect newDockedDividerFrame =
        NSMakeRect(newGroupX, runningDividerFrame.origin.y, runningDividerFrame.size.width,
                   runningDividerFrame.size.height);
    [self.runningDivider setFrame:newDockedDividerFrame];
  ...
  if (self.trashGroup) {
    ...
    NSRect newDockedFrame =
        NSMakeRect(newGroupX, trashFrame.origin.y, trashFrame.size.width, trashFrame.size.height);
  ...
  NSRect newFrame =
      NSMakeRect(newX, self.padding, currentFrame.size.width, currentFrame.size.height);
...
- (void)iconDropped:(NSString *)appName inGroup:(DockGroup *)dockGroup
{
  ...
  if (dockGroup.acceptsIcons) {
    [dockGroup addIcon:appName
             withImage:[self.workspace appIconForApp:appName]
               atIndex:[[dockGroup listIconNames] count]];
  }
...
- (void)iconRemovedFromWindow:(NSNotification *)notification
{
  ...
  if (dockGroup.canDragRemove) {
    ...
    if (self.showRunning && self.runningGroup && isRunning) {
      ...
      [self.runningGroup addIcon:appName
                       withImage:appImage
                         atIndex:[[self.runningGroup listIconNames] count]];
...
- (void)iconIsDragging:(NSNotification *)notification
{
  ...
  if (isOverDocked) {
    ...
    if (![self.dropTarget hasIcon:appName]) {
      ...
      [self.dropTarget addIcon:appName
                     withImage:draggedIcon.iconImage
  ...
  if (isOverRunning) {
    ...
    if (![parentGroupName isEqualToString:[self.dockedGroup getGroupName]] &&
        [self.dockedGroup hasIcon:appName]) {
