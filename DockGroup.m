...
- (void)updateFrame
{
  ...
  NSSize newContentSize = [_direction isEqualToString:@"Horizontal"]
                              ? NSMakeSize(dockWidth, self.iconSize)
                              : NSMakeSize(self.iconSize, dockWidth);
  NSRect currentFrame = [self frame];
  NSRect newFrame =
      NSMakeRect(currentFrame.origin.x, 0, newContentSize.width, newContentSize.height);
...
- (void)swapIconPositions:(NSUInteger)draggedIndex withEmptyIndex:(NSUInteger)emptyIndex
{
  ...
  if (swapHorizontal) {
    ...
    NSRect aNewFrame =
        NSMakeRect(bFrame.origin.x, bFrame.origin.y, aFrame.size.width, aFrame.size.height);
    NSRect bNewFrame =
        NSMakeRect(aFrame.origin.x, aFrame.origin.y, aFrame.size.width, aFrame.size.height);
  ...
  [self.dockedIcons replaceObjectAtIndex:draggedIndex
                              withObject:[self.dockedIcons objectAtIndex:emptyIndex]];
...
{
  // If isDocked, we need to move subset of dockedIcons and all of the undockedIcons so we create a
  // global array. Otherwise we move subset of undockedIcons only.
    ...
    if ([self.dockPosition isEqualToString:@"Bottom"]) {
      ...
      if (isExpanding) {
        ...
        NSRect expandedFrame =
            NSMakeRect(expandedX, currentFrame.origin.y, self.iconSize, self.iconSize);
        [dockIcon setFrame:expandedFrame]; // Replace with tween
      }
      else {
        CGFloat contractedX = currentFrame.origin.x - self.iconSize;
        NSRect contractedFrame =
            NSMakeRect(contractedX, currentFrame.origin.y, self.iconSize, self.iconSize);
...
{
  NSUInteger index =
      [self.dockedIcons indexOfObjectPassingTest:^BOOL(id obj, NSUInteger idx, BOOL *stop) {
...
- (NSRect)generateLocation:(NSString *)dockPosition atIndex:(CGFloat)index
{
  if ([dockPosition isEqualToString:@"Left"]) {
    NSRect leftLocation =
        NSMakeRect(self.activeLight, [self.dockedIcons count] * self.iconSize + (self.padding),
                   self.iconSize, self.iconSize);
    return leftLocation;
  }
  else if ([dockPosition isEqualToString:@"Right"]) {
    NSRect rightLocation =
        NSMakeRect(self.activeLight, [self.dockedIcons count] * self.iconSize + (self.padding),
                   self.iconSize, self.iconSize);
    ...
    NSRect bottomLocation =
        NSMakeRect(index * self.iconSize, self.activeLight, self.iconSize, self.iconSize);
...
- (NSDragOperation)draggingEntered:(id<NSDraggingInfo>)sender
{
  ...
  if ([[pasteboard types] containsObject:NSFilenamesPboardType]) {
    ...
    NSString *appBundleName =
        [[[draggedFilenames objectAtIndex:0] lastPathComponent] stringByDeletingPathExtension];
...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)sender
{
  ...
  if ([[pasteboard types] containsObject:NSFilenamesPboardType]) {
    ...
    if (self.acceptsIcons && [[[draggedFilenames objectAtIndex:0] pathExtension] isEqual:@"app"] &&
        !alreadyDocked) {
